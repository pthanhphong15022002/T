...
export class CalendarDateComponent implements OnInit, OnChanges {
  ...
  getDateFromWeekNum(weekNo, yearNo) {
    ...
    if (firstDayofYear.getDay() > 4) {
      let weekStart = new Date(
        yearNo,
        0,
        (weekNo - 1) * 7 - firstDayofYear.getDay() + 8
      );
      let weekEnd = new Date(
        yearNo,
        0,
        1 + (weekNo - 1) * 7 - firstDayofYear.getDay() + 8 + 5
      );
      ...
      let weekStart = new Date(
        yearNo,
        0,
        (weekNo - 1) * 7 - firstDayofYear.getDay() + 1
      );
      let weekEnd = new Date(
        yearNo,
        0,
  ...
  selectDate(event, mode) {
    ...
    if (this.tabIndex != 1) {
      ...
      if (this.tabIndex == 3) this.monthFull = '';
      else if (this.tabIndex == 4) {
        this.monthFull = `${this.fromDate.toLocaleString().split(',')[0]} - ${
          this.toDate.toLocaleString().split(',')[0]
        }`;
        this.year = '';
      }
  ...
  goYear(mode) {
    if (mode == 'up') this.year++;
    else this.year--;
    this.weeks = this.weeksInYear();
    if (this.weeks > this.week) this.week = this.weeks;
