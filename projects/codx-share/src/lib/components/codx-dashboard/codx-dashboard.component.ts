...
export class CodxDashboardComponent implements OnInit, AfterViewInit {
  ...
  savePanel(evt: any) {
    ...
    arrPanels.forEach((item: any) => {
      let itemSetting = this.objDashboard.panels.filter(
        (x: any) => x.id == item.id
      )[0] as any;
      if (itemSetting) {
        ...
        if (itemSetting.isTemplate) {
          ...
          objPanelSetting.data = itemSetting.chartSetting
            ? JSON.stringify(itemSetting.chartSetting)
            : '';
        }
    ...
    console.log(
      JSON.stringify(dataSettings.panels, function replacer(key, value) {
        return value;
      })
    );
    console.log(
      JSON.stringify(dataSettings.panelDatas, function replacer(key, value) {
        return value;
      })
    );
  }
      ...
      if (evt.template) {
        ...
        if (!chartInfo && (this.objDashboard.panels[idx] as any).chartType) {
          ...
          if ((this.objDashboard.panels[idx] as any).chartSetting) {
            ...
            if (_seriesSetting.legendSetting)
              delete _seriesSetting.legendSetting;
        ...
        dialog.closed.subscribe((res) => {
          ...
          if (res.event) {
            ...
            if (idx > -1) {
              ...
              (this.objDashboard.panels[idx] as any).chartSetting = JSON.parse(
  ...
  onResizeStop(args: any) {
    ...
    if (args.element) {
      ...
      if (args.element.querySelector('ejs-chart')) {
        const chartObj =
          args.element.querySelector('ejs-chart').ej2_instances[0];
        chartObj.height = '80%';
        chartObj.width = '100%';
        ...
        if (
          ...
          this.panels.forEach((ele: any) => {
            ...
            if (panelChart) {
              ...
              if (this.isJSON(panelChart.data))
                panelData = JSON.parse(panelChart.data);
              if (panelData.serieSetting) {
                if (idx > -1) {
                  (this.objDashboard.panels[idx] as any).chartSetting =
                    panelData;
                  (this.objDashboard.panels[idx] as any).chartType =
                    panelData.serieSetting.type;
                }
  ...
  private addDefaultChart(chartType: string, panelID: string) {
    ...
    switch (chartType) {
      case 'Line':
        (selectedTemplate = this.dfLine), (defaultSetting = this.lineSettings);
        break;
      case 'StepLine':
      ...
      case 'MultiColoredLine':
        ...
        break;
      case 'Area':
        (selectedTemplate = this.dfArea), (defaultSetting = this.areaSettings);
        break;
      case 'RangeArea':
      ...
      case 'StackingColumn100':
        ...
        break;
      case 'Bar':
        (selectedTemplate = this.dfBar), (defaultSetting = this.barSettings);
        break;
      case 'StackingBar':
      ...
      case 'Radar':
        ...
        break;
      case 'Hilo':
        (selectedTemplate = this.dfHilo), (defaultSetting = this.hiloSettings);
        break;
      case 'Histogram':
      ...
      case 'Pareto':
        ...
        break;
      case 'Pie':
        (selectedTemplate = this.dfPie), (defaultSetting = this.pieSettings);
        break;
      case 'Pyramid':
  ...
  dataStepLine: Object[] = [
    {
      Period: 'ONE',
      CHN_UnemploymentRate: 16,
      AUS_UnemploymentRate: 35,
      ITA_UnemploymentRate: 3.4,
    },
    {
      Period: 'TWO',
      CHN_UnemploymentRate: 12.5,
      AUS_UnemploymentRate: 45,
      ITA_UnemploymentRate: 4.4,
    },
    {
      Period: 'THREE',
      CHN_UnemploymentRate: 19,
      AUS_UnemploymentRate: 55,
      ITA_UnemploymentRate: 6,
    },
    {
      Period: 'FOUR',
      CHN_UnemploymentRate: 14.4,
      AUS_UnemploymentRate: 20,
      ITA_UnemploymentRate: 7,
    },
    {
      Period: 'FIVE',
      CHN_UnemploymentRate: 11.5,
      AUS_UnemploymentRate: 10,
      ITA_UnemploymentRate: 11.3,
    },
    {
      Period: 'SIX',
      CHN_UnemploymentRate: 14,
      AUS_UnemploymentRate: 42,
      ITA_UnemploymentRate: 10.1,
    },
    {
      Period: 'SEVEN',
      CHN_UnemploymentRate: 10,
      AUS_UnemploymentRate: 35,
      ITA_UnemploymentRate: 7.8,
    },
    {
      Period: 'EIGHT',
      CHN_UnemploymentRate: 16,
      AUS_UnemploymentRate: 22,
      ITA_UnemploymentRate: 8.5,
    },
    {
      Period: 'NINE',
      CHN_UnemploymentRate: 16,
      AUS_UnemploymentRate: 65,
      ITA_UnemploymentRate: 8.5,
    },
    {
      Period: 'TEN',
  ...
  load(args: any): void {
    ...
    [
      150, 71.5, 106.4, 100.25, 70.0, 106.0, 85.6, 78.5, 76.4, 86.1, 155.6,
      160.4,
  ...
  dataStackingArea: Object[] = [
    {
      Period: new Date(2000, 1, 1),
      OrganicSales: 0.61,
      FairTradeSales: 0.03,
      VegAlternativesSales: 0.48,
      OtherSales: 0.23,
    },
    {
      Period: new Date(2002, 1, 1),
      OrganicSales: 0.91,
      FairTradeSales: 0.06,
      VegAlternativesSales: 0.57,
      OtherSales: 0.17,
    },
    {
      Period: new Date(2004, 1, 1),
      OrganicSales: 1.19,
      FairTradeSales: 0.14,
      VegAlternativesSales: 0.63,
      OtherSales: 0.23,
    },
    {
      Period: new Date(2006, 1, 1),
      OrganicSales: 1.74,
      FairTradeSales: 0.29,
      VegAlternativesSales: 0.66,
      OtherSales: 0.43,
    },
    {
      Period: new Date(2008, 1, 1),
      OrganicSales: 1.99,
      FairTradeSales: 0.64,
      VegAlternativesSales: 0.77,
      OtherSales: 0.72,
    },
    {
      Period: new Date(2010, 1, 1),
      OrganicSales: 1.48,
      FairTradeSales: 1.06,
      VegAlternativesSales: 0.54,
      OtherSales: 1.38,
    },
    {
      Period: new Date(2012, 1, 1),
      OrganicSales: 1.66,
      FairTradeSales: 1.55,
      VegAlternativesSales: 0.61,
      OtherSales: 2.16,
    },
    {
  ...
  dataBar: Object[] = [
    ...
    { Country: 'Italy', GDP: 1.5, WorldShare: 2.4 },
  ...
  dataPyramid: Object[] = [
    {
      Foods: 'Milk, Youghnut, Cheese',
      Calories: 435,
      DataLabelMappingName: 'Milk, Youghnut, Cheese: 435 cal',
    },
    {
      Foods: 'Vegetables',
      Calories: 470,
      DataLabelMappingName: 'Vegetables: 470 cal',
    },
    {
      Foods: 'Meat, Poultry, Fish',
      Calories: 475,
      DataLabelMappingName: 'Meat, Poultry, Fish: 475 cal',
    },
    { Foods: 'Fruits', Calories: 520, DataLabelMappingName: 'Fruits: 520 cal' },
    {
      Foods: 'Bread, Rice, Pasta',
      Calories: 930,
      DataLabelMappingName: 'Bread, Rice, Pasta: 930 cal',
    },
  ];
  neckWidth: string = '15%';
