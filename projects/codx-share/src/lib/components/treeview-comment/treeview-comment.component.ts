...
export class TreeviewCommentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getValueIcon();
    if (this.showComment) this.showComments();
  }
      ...
      this.api
        ...
        .subscribe((res: any[]) => {
          ...
          if (res) {
            ...
            if (!data.totalPage) data.totalPage = Math.ceil(res[1] / 5);
  ...
  getUserVotes(postID: string, voteType: String) {
    this.api
      .execSv('WP', 'ERM.Business.WP', 'VotesBusiness', 'GetUserVotesAsync', [
        postID,
        voteType,
      ])
      .subscribe((res) => {
  ...
  setNodeTree(newNode: any) {
    ...
    if (parent) this.addNode(parent, newNode, id);
    else this.addNode(null, newNode, id);
    this.dt.detectChanges();
  }
  // add tree
  addNode(dataNode: any, newNode: any, id: string) {
    ...
    if (dataNode) {
      ...
      } else dataNode.listComment.unshift(newNode);
    } else {
      if (!Array.isArray(this.data.listComment)) this.data.listComment = [];
  ...
  removeNodeTree(id: string) {
    ...
    if (data) {
      ...
      if (parent)
        parent.listComment = parent.listComment.filter(
          (element: any) => element['recID'] != id
        );
      else
        this.data.listComment = this.data.listComment.filter(
          (element: any) => element['recID'] != id
        );
