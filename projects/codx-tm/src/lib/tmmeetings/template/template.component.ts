...
import { DialogData, DialogRef, ApiHttpService, UIComponent, AlertConfirmInputConfig, NotificationsService } from 'codx-core';
...
export class TemplateComponent extends UIComponent implements OnInit {
  ...
  onSave(){
    if(this.data == null){
      ...
      config.type = "YesNo";
      this.notificationsService.alert('Thông báo','Đã có dữ liệu cũ bạn có chắc chắn muốn chỉnh sửa?',config).closed
      .subscribe(x=>{
        if(x.event.status == 'Y'){
          this.dialog.close(this.tempID);
        }
