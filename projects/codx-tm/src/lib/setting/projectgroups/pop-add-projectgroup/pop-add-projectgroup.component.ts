...
export class PopAddProjectgroupComponent implements OnInit {
  ...
  title = '';
  titleAdd = 'Thêm mới nhóm dự án';
  titleUpdate = 'Chỉnh sửa nhóm dự án';
  dialog: any;
  user: any;
  functionID: string;
  gridViewSetup: any;
  ...
  ngOnInit(): void {
    this.cache
      .gridViewSetup('ProjectGroups', 'grvProjectGroups')
      .subscribe((res) => {
  ...
  initForm() {
    this.getFormGroup(this.formName, this.gridViewName).then((item) => {
      ...
      if (this.projectGroups.projectGroupID == undefined) {
