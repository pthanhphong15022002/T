...
import { Approver, ResponseModel } from 'projects/codx-common/src/lib/models/ApproveProcess.model';
import { GridModels } from '../../models/resource.model';
@Component({
  selector: 'ep-add-booking-stationery',
  templateUrl: './ep-add-booking-stationery.component.html',
  styleUrls: ['./ep-add-booking-stationery.component.scss'],
})
export class EpAddBookingStationeryComponent extends UIComponent {
  ...
  onInit(): void {
    ...
    this.codxEpService.getStationeryGroup().subscribe((res) => {
          ...
          if (approval) {
            ...
            if (!autoRelease) {
              ...
              for(let i=0; i<this.returnData?.length;i++){
                this.codxEpService.approvedManual(this.returnData[i]?.recID).subscribe((approveData:any)=>{
                  if(approveData!=null){
