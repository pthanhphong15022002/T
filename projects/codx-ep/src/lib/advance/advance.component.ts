...
import { PopupAddAdvanceComponent } from './popup-add-advance/popup-add-advance.component';
  ...
  implements AfterViewInit, OnDestroy
{
  ...
  @ViewChild('advanceDetail') advanceDetail: AdvanceDetailComponent;
  constructor(
  ...
  add() {
    this.view.dataService.addNew().subscribe((data: any) => {
      if (data) {
        ...
        data.positionName = this.user.employee?.positionName;
        ...
        this.subcriptions.add(
          popup.closed.subscribe((res: any) => {
            if (res?.event) {
              let subcribeAdd = this.view.dataService
                .update(res.event)
                .subscribe();
  ...
  delete(data: any) {
    if (data) {
      let subcribeDelete = this.view.dataService
        .delete([data], true)
        .subscribe();
  ...
  update(data: any) {
    if (data) {
      ...
      this.subcriptions.add(
        popup.closed.subscribe((res: any) => {
          if (res?.event?.recID) {
            this.advanceDetail.loadRequestDetail(res.event.recID);
          }
        })
      );
    }
  ...
  coppy(data: any) {
    if (data) {
      let subcribeCoppy = this.view.dataService.copy().subscribe((res: any) => {
        if (res) {
          let dialog = new SidebarModel();
          dialog.Width = '550px';
          dialog.FormModel = this.view.formModel;
  ...
  updateStatus(data: any) {
    if (data) {
      let subcribeApi = this.api
        .execSv('EP', 'EP', 'RequestsBusiness', 'UpdateStatusAsync', data.recID)
        .subscribe((res: any) => {
          if (res) {
            this.view.dataService.dataSelected.status = '5';
            this.advanceDetail.loadRequestDetail(
