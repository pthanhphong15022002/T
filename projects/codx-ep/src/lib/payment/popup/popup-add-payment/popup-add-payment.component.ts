  ...
  implements OnInit, AfterViewInit, OnDestroy
{
        ...
        if (!this.data.currencyID) {
          this.subcriptions.add(
            this.cache.companySetting().subscribe((res: any) => {
              if (res) this.data.currencyID = res[0]['baseCurr'] || '';
            })
          );
        }
    ...
    let subcribeApi = this.api
      ...
      .subscribe((res: any) => {
        if (res) {
          ...
          if (this.data.lines)
            this.data.totalAmount = this.data.lines.reduce(
              (accumulator, currentValue) => accumulator + currentValue.amount,
              0
            );
          else {
  ...
  getAdvanceRequest(refID: string, requestType: string) {
    let subcribe = this.api
      .execSv('EP', 'EP', 'RequestsBusiness', 'GetRequestByIDTypeAsync', [
        refID,
        requestType,
      ])
      .subscribe((res: any) => {
  ...
  valueCellChange(event: any) {
    ...
    if (field == 'amount') {
      ...
      if (this.RefEPRequest)
        this.data.requestAmt =
          this.data.requestAmt - this.RefEPRequest.requestAmt;
    }
    this.detectorChange.detectChanges();
  }
