...
import { DialogRef, CodxGridviewV2Component, LayoutAddComponent, ApiHttpService, CacheService, AuthStore, NotificationsService, DialogData, Util } from 'codx-core';
...
export class PopupAddBusinesstripComponent implements OnInit,AfterViewInit,OnDestroy {
  ...
  ) 
  {
    ...
    if(dialogData?.data)
    {
      ...
      if(this.actionType == "add")
      {
        ...
        this.EPBookingAttendeeIDs = this.data.resources.map(x => x.userID).join(";");
  ...
  ngOnInit(): void {
    this.subcriptions.add(this.cache.valueList("EP010")
    .subscribe((vll:any) => {
      if(vll)
      {
    ...
    .subscribe((vll:any) => {
      if(vll)
        vll.datas.forEach(item => this.ACObjectTypes.set(item.value.toString(),item.text));
    }));
  }
    ...
    .subscribe((res:any) => {
      if(res)
      {
        ...
        if(this.data.resources?.length > 0)
          this.EPBookingAttendeeIDs = this.data.resources.filter(x => x.roleType != "2").map(x => x.userID).join(";");
  ...
  getEPResource(resourceID){
    let subcribeApi = this.api.execSv("EP","EP","ResourcesBusiness","GetResourceAsync",resourceID)
    .subscribe((res:any) => {
      this.EPResource = res;
      this.detectorChange.detectChanges();
    });
      ...
      case "objectID":
        ...
        if(event.component.itemsSelected.length > 0)
        {
          ...
          if(this.ACObjectTypes)
            this.data.memo = this.ACObjectTypes.get(this.ACObject.ObjectType) + " "; 
      ...
      case "resourceID":
        ...
        if(value)
        {
          this.EPResource = {
            ...
            code : value.Code,
            equipments : value.Equipments ? value.Equipments.map(item => ({ equipmentID: item.EquipmentID, createdBy: item.CreatedBy, createdOn: item.CreatedOn})) : []
          };
  ...
  valueCellChange(event:any){
    let field = event.field;
    if(field == "itemID") 
      event.data.itemName = event.itemData.CostItemName;
    let EPRequestsLine = {
  ...
  onSave(isRelease:boolean = false){
    ...
    this.layoutAdd.form.save(null, 0, '', '', true,{allowCompare:false})
    .subscribe((res:any) => {
