...
export class PropertyCbbDependenceComponent extends BasePropertyComponent implements OnChanges , AfterViewInit{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    {
      this.vaidateControl = typeof this.data?.validateControl == 'string' ? JSON.parse(this.data.validateControl) : this.data.validateControl;
    ...
    dtTable.forEach(element => {
      if(element.columnOrder <= this.data.columnOrder)
      {
        ...
        element.children.forEach(elm2=>{
          ...
          {
            var obj =  { id: elm2.refValue , text: elm2.title , columnOrder: elm2.columnOrder , columnNo: elm2.columnNo};
  ...
  changeValueCCBB(dt:any)
  {
    ...
    let de = this.dataTable[this.dtCbb.columnOrder].children[this.dtCbb.columnNo]?.dependences
    if(!de) de = str
    else {
