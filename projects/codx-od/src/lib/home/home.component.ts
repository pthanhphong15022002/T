...
export class HomeComponent implements OnInit, AfterViewInit {
    ...
    if (data != null && data.length > 0) {
      if (overdate == '') {
        ...
        if (this.odService.DataChart != null)
          data = this.odService.DataChartOld.filter(
            (x) => x.status == '1' && x.deadline < date
          );
      } else if (overdate == 'urgency') {
        data = this.odService.DataChartOld.filter(
          (x) => x.status == '1' && x.urgency == '5'
        );
      }
      this.odService.DataChart = data;
      this.odService.ChangeData.next(true);
  ...
  GetTabSelected(tab) {
    ...
    else return 'tm-box box-primary w-200px p-6 border-bottom cursor-pointer';
  }
  ...
  constructor(
    public odService: DispatchService,
  ...
  GetChartLine() {
    ...
    // x x
    if (
      this.odService.DataChart7 != null &&
      this.odService.DataChart7.length > 0
    ) {
  ...
  FilterDataChart() {
    ...
    if (
      this.odService.DataChart9 != null &&
      this.odService.DataChart9.length > 0
    ) {
  ...
  LoadDataToLineChart() {
    var that = this;
    this.odService
      .getDataForCharts(this.fromDate, this.toDate, this.view, this.type)
      .subscribe((item) => {
  ...
  onLoadDate(args: any) {
    ...
    this.date = yyyy + mm;
    if (args.view.toString() != 'Month') this.date = yyyy;
  ...
  CountByAmergency() {
    var len = 0;
    if (this.odService.DataChart != null)
      len = this.odService.DataChart.filter(
        (x) => x.status == '1' && x.urgency == '5'
      ).length;
    return len;
  }
