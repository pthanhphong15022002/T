...
export class ViewDetailComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  openFormFuncID(val: any, datas: any = null , isData = false) {
    ...
    switch (funcID) {
      ...
      case "ODT3001" : {
        if(isData)
        {
          this.odService.getDetailDispatch(datas.recID,this.formModel.entityName).subscribe((item) => {
            if (item) {
  ...
  cancelAproval(data:any)
  {
    ...
    if(data.bsCategory)
    {
      this.api.execSv("ES","ES","SignFilesBusiness","CancelSignfileAsync",data.recID).subscribe(item=>{
        if(item)
        {
