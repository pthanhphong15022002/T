...
export class ViewFileDialogComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes['dataFile'] &&
      changes['dataFile']?.currentValue != changes['dataFile']?.previousValue
    ) {
  ...
  viewFile(id) {
    ...
    if (this.data?.extension.includes('doc')) {
      ...
      this.urlTxt = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.linkFile
      );
    }
  }
  ...
  getData() {
    ...
    baseurlExcel +=
      '?sk=' +
      btoa(this.auth.userValue.userID + '|' + this.auth.userValue.securityKey);
        ...
        if (msg.MessageId == 'App_LoadingStatus') {
          if (msg.Values) {
            if (msg.Values.Status == 'Document_Loaded') {
              console.log(
                '==== inform the wopi client that we are ready to receife messages'
              );
              window.frames[0].postMessage(
              ...
              window.frames[0].postMessage(
                JSON.stringify({ MessageId: 'Hide_Menubar' }),
                '*'
              );
