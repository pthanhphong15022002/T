...
export class PopupPolicyalComponent extends UIComponent implements OnInit {
  ...
  @ViewChild('headTmpGrid2Col1', { static: true })
  headTmpGrid2Col1: TemplateRef<any>;
  @ViewChild('headTmpGrid2Col2', { static: true })
  headTmpGrid2Col2: TemplateRef<any>;
        ...
        this.DeletePolicyDetail(data.recID).subscribe((res) => {
          if (res == true) {
            ...
            if (gridNum == 1) {
              ...
              (this.gridView2?.dataService as CRUDService)
                ?.remove(data)
                .subscribe();
        ...
        if (this.alpolicyObj.hasExcludeObjects == true) {
          if (this.alpolicyObj.excludeObjects) {
            this.lstSelectedExcludeObj =
              this.alpolicyObj.excludeObjects.split(';');
          }
        }
        this.GetPolicyBeneficiaries(this.alpolicyObj.policyID).subscribe(
          (res) => {
  ...
  async onSaveForm() {
    ...
    if (
      ...
      (this.alpolicyObj.includeObjects?.length < 1 ||
    ...
    if (
      ...
      (this.alpolicyObj.excludeObjects?.length < 1 ||
    ...
    if (this.actionType === 'add' || this.actionType === 'copy') {
      ...
      if (
        ...
        this.EditPolicyAL(this.alpolicyObj).subscribe((res) => {
          if (res) {
            this.notify.notifyCode('SYS007');
            this.DeletePolicyBeneficiaries(this.alpolicyObj.policyID).subscribe(
              (res) => {
                if (
                  this.alpolicyObj.hasIncludeObjects == true ||
                  this.alpolicyObj.hasExcludeObjects == true
  ...
  onEditGrid2(evt) {
    ...
    let index = this.gridView2.dataSource.findIndex(
      (v) => v.recID == evt.recID
    );
    this.UpdatePolicyDetail(evt).subscribe((res) => {
      if (res && res.oldData) {
