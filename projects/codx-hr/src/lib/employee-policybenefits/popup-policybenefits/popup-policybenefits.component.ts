  ...
  implements OnInit
{
  ...
  @ViewChild('headTmpGrid1Col3', { static: true })
  headTmpGrid1Col3: TemplateRef<any>;
  ...
  onInit(): void {
    ...
    this.cache
      .gridViewSetup('PolicyConstraints', 'grvPolicyConstraints')
      .subscribe((res) => {
    ...
    this.hrSevice
      .getFormGroup(
        this.fmPolicyConstraints.formName,
        this.fmPolicyConstraints.gridViewName,
        this.fmPolicyConstraints
      )
      .then((fg) => {
        if (fg) {
  ...
  addRowGrid1() {
    if (this.benefitPolicyObj.policyID || this.form.form.data.policyID) {
      this.CheckIfPolicyIDExist(this.benefitPolicyObj.policyID).subscribe(
        (res) => {
          if (res[0] == true) {
            // this.originPolicyId = this.benefitPolicyObj.policyID;
  ...
  onEditGrid1(evt) {
    if (!evt.fromValue) {
      ...
      setTimeout(() => {
        this.GetPolicyDetailByAjustBy(
          this.form.form.preData.policyID,
          this.benefitPolicyObj.adjustBy
        ).subscribe((res) => {
    ...
    if (this.benefitPolicyObj.isConstraintOther) {
      ...
      let popup = this.callfunc.openForm(
        ...
        null,
        {
          ...
          vllName: this.form.form.gridviewSetup.ConstraintBy.referedValue,
      ...
      if (x.event?.status == 'Y') {
        if (this.currentTab == 'applyObj') {
          ...
          if (index != -1) {
            ...
            this.lstPolicyBeneficiariesApply = this.hrSevice.sortAscByProperty(
          ...
          if (index != -1) {
            ...
            this.lstPolicyBeneficiariesExclude =
              this.hrSevice.sortAscByProperty(
                this.lstPolicyBeneficiariesExclude,
                'priority'
              );
  ...
  onClickMoveObjUp(obj) {
    ...
    if (this.currentTab == 'applyObj') {
      objBeforeIndex = this.lstPolicyBeneficiariesApply.findIndex(
        (p) => p.priority == crrPriority - 1
      );
      obj.priority -= 1;
      if (objBeforeIndex > -1) {
      ...
      objBeforeIndex = this.lstPolicyBeneficiariesExclude.findIndex(
        (p) => p.priority == crrPriority - 1
      );
      obj.priority -= 1;
      if (objBeforeIndex > -1) {
    ...
    if (this.benefitPolicyObj.hasIncludeObjects) {
      ...
      popup.closed.subscribe((res) => {
        if (res.event) {
          ...
          if (
            this.lstSelectedObj.length > 0 &&
            this.lstPolicyBeneficiariesApply.length < 1
          ) {
            this.addApplyObj();
          }
        } else {
    ...
    if (this.benefitPolicyObj.hasExcludeObjects) {
      ...
      popup.closed.subscribe((res) => {
        if (res.event) {
          ...
          if (
            this.lstSelectedExcludeObj.length > 0 &&
            this.lstPolicyBeneficiariesExclude.length < 1
          ) {
            this.addExcludeObj();
          }
        } else {
  ...
  onClickOpenSelectApplyObjDetail(detail, obj) {
    ...
    if (this.benefitPolicyObj.hasIncludeObjects) {
      if (detail == '5' || detail == '8') {
        ...
        popup.closed.subscribe((res) => {
          let index = this.lstPolicyBeneficiariesApply.findIndex(
            (x: any) => this.currentRec == x['recID']
          );
          let lstId = res.event.split(';');
          if (vll == 'HR022') {
            this.lstPolicyBeneficiariesApply[index].labourType = res.event;
            this.lstLabourType = this.lstLabourType.filter(
              (obj) => obj.rec != this.currentRec
            );
            ...
            this.lstEmpStatus = this.lstEmpStatus.filter(
              (obj) => obj.rec != this.currentRec
            );
            for (let i = 0; i < lstId.length; i++) {
  ...
  onClickOpenSelectExcludeObjDetail(detail, obj) {
    ...
    if (this.benefitPolicyObj.hasExcludeObjects) {
      if (detail == '5' || detail == '8') {
        ...
        popup.closed.subscribe((res) => {
          let index = this.lstPolicyBeneficiariesExclude.findIndex(
            (x: any) => this.currentRec == x['recID']
          );
          let lstId = res.event.split(';');
          if (vll == 'HR022') {
            ...
            this.lstPolicyBeneficiariesExclude[index].employeeStatus =
              res.event;
            this.lstEmpStatus = this.lstEmpStatus.filter(
              (obj) => obj.rec != this.currentRec
            );
  ...
  initForm() {
    ...
    if (this.actionType == 'add') {
      ...
      this.hrSevice
        ...
        .subscribe((res: any) => {
          if (res) {
            ...
            // this.formModel.currentData = this.benefitPolicyObj;
            // this.formGroup.patchValue(this.benefitPolicyObj);
            // console.log('formgroup sau khi patch val', this.formGroup);
      ...
      if (this.actionType === 'edit' || this.actionType === 'copy') {
        ...
        if (this.benefitPolicyObj.isConstraintOther) {
          this.lstSelectedConstraintBy =
            this.benefitPolicyObj.constraintBy.split(';');
          this.lstSelectedConstraintBy.sort((a, b) => {
            return parseInt(a) - parseInt(b);
          });
          this.GetPolicyConstraint(this.benefitPolicyObj.policyID).subscribe(
            (res) => {
        ...
        if (this.benefitPolicyObj.hasIncludeBenefits == true) {
          this.lstSelectedBenefits =
            this.benefitPolicyObj?.includeBenefits?.split(';');
        }
        ...
        if (this.benefitPolicyObj.hasIncludeObjects == true) {
          if (this.benefitPolicyObj.includeObjects) {
            this.lstSelectedObj =
              this.benefitPolicyObj.includeObjects.split(';');
          }
        }
        ...
        if (this.benefitPolicyObj.hasExcludeObjects == true) {
          if (this.benefitPolicyObj.excludeObjects) {
            this.lstSelectedExcludeObj =
              this.benefitPolicyObj.excludeObjects.split(';');
          }
        }

        this.GetPolicyBeneficiaries(this.benefitPolicyObj.policyID).subscribe(
          (res) => {
            this.lstPolicyBeneficiariesApply = res.filter(
              (obj) => obj.category == 0
            );
            this.lstPolicyBeneficiariesApply = this.hrSevice.sortAscByProperty(
              this.lstPolicyBeneficiariesApply,
            ...
            this.lstPolicyBeneficiariesExclude = res.filter(
              (obj) => obj.category == 1
            );
            this.lstPolicyBeneficiariesExclude =
              this.hrSevice.sortAscByProperty(
    ...
    if (this.constraintsObj) {
      if (
        ...
        this.lstSelectedConstraintTrainLevel =
          this.constraintsObj.trainLevel.split(';');
      }
      if (
        this.constraintsObj.trainField &&
        this.constraintsObj.trainField != ''
      ) {
        this.lstSelectedConstraintTrainField =
          this.constraintsObj.trainField.split(';');
      }
      if (
        this.constraintsObj.certificate &&
        this.constraintsObj.certificate != ''
      ) {
        this.lstSelectedConstraintCertificate =
          this.constraintsObj.certificate.split(';');
      }
    }
  }
  ...
  onClickOpenComboboxConstraint(target) {
    if (target == 'trainLevel') {
      ...
      //cbx
      this.cbxConstraintCbxName =
        this.grvSetupPolicyConstraint.TrainField.referedValue;
      this.cbxConstraintCbxValue = this.constraintsObj.trainField;
      this.isHiddenCbxConstraint = false;
    } else if (target == 'certificate') {
      //cbx
      this.cbxConstraintCbxName =
        this.grvSetupPolicyConstraint.Certificate.referedValue;
  ...
  onClickHideComboboxPopup(event) {
    ...
    if (event == null) {
      ...
      let index = 0;
      if (this.currentTab == 'applyObj') {
        index = this.lstPolicyBeneficiariesApply.findIndex(
          (x: any) => this.currentRec == x['recID']
        );
      } else if (this.currentTab == 'subtractObj') {
        index = this.lstPolicyBeneficiariesExclude.findIndex(
          (x: any) => this.currentRec == x['recID']
        );
      }
      ...
      switch (this.currentCbxName) {
        case 'HRDepts':
          ...
          this.lstOrgUnitID = this.lstOrgUnitID.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
            this.lstOrgUnitID.push({
        ...
        case 'JobLevels':
          ...
          this.lstJobLevel = this.lstJobLevel.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
            this.lstJobLevel.push({
        ...
        case 'PositionsName':
          ...
          this.lstPositionID = this.lstPositionID.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
            this.lstPositionID.push({
        ...
        case 'EmployeeTypes':
          ...
          this.lstEmployeeTypeID = this.lstEmployeeTypeID.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
            this.lstEmployeeTypeID.push({
        ...
        case 'ContractTypes':
          ...
          this.lstContractTypeID = this.lstContractTypeID.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
            this.lstContractTypeID.push({
        ...
        case 'Employees':
          ...
          this.lstEmployeeID = this.lstEmployeeID.filter(
            (obj) => obj.rec != this.currentRec
          );
          for (let i = 0; i < lstId.length; i++) {
  ...
  onClickMoveObjDown(obj) {
    ...
    if (this.currentTab == 'applyObj') {
      objAfterIndex = this.lstPolicyBeneficiariesApply.findIndex(
        (p) => p.priority == crrPriority + 1
      );
      obj.priority += 1;
      if (objAfterIndex > 0) {
      ...
      objAfterIndex = this.lstPolicyBeneficiariesExclude.findIndex(
        (p) => p.priority == crrPriority + 1
      );
      obj.priority += 1;
      if (objAfterIndex > 0) {
  ...
  async onSaveForm() {
    ...
    if (
      this.benefitPolicyObj.hasIncludeBenefits == true &&
      !this.benefitPolicyObj.includeBenefits
    ) {
    ...
    if (
      this.benefitPolicyObj.hasIncludeObjects == true &&
      (this.benefitPolicyObj.includeObjects?.length < 1 ||
        this.lstPolicyBeneficiariesApply.length < 1)
    ) {
    ...
    if (
      this.benefitPolicyObj.hasExcludeObjects == true &&
      (this.benefitPolicyObj.excludeObjects?.length < 1 ||
        this.lstPolicyBeneficiariesExclude.length < 1)
    ) {
    ...
    if (this.actionType === 'add' || this.actionType === 'copy') {
      ...
      if (
        ...
        this.DeletePolicyBenefit(this.form.form.preData.policyID).subscribe(
          (x) => {
            this.AddPolicyBenefits(this.benefitPolicyObj).subscribe((res) => {
              if (res) {
                this.notify.notifyCode('SYS007');
                ...
                this.benefitPolicyObj.oldData = this.form.form.preData;
                this.dialog && this.dialog.close(this.benefitPolicyObj);
              } else {
  ...
  deleteApplyExcludeObjMain(data, from, lstBeneficiaries) {
    ...
    switch (from) {
      ...
      case 'lstSelectedExcludeObj':
        ...
        this.benefitPolicyObj.excludeObjects =
          this.lstSelectedExcludeObj.join(';');
        break;
    }
  }
  ...
  deleteApplyExcludeObj(data, from, crrObj?) {
    switch (from) {
      ...
      case 'lstSelectedConstraintTrainLevel':
        ...
        this.constraintsObj.trainLevel =
          this.lstSelectedConstraintTrainLevel.join(';');
        break;
      case 'lstSelectedConstraintTrainField':
        ...
        this.constraintsObj.trainField =
          this.lstSelectedConstraintTrainField.join(';');
        break;
      case 'lstSelectedConstraintCertificate':
        ...
        this.constraintsObj.certificate =
          this.lstSelectedConstraintCertificate.join(';');
        break;
    }
    this.df.detectChanges();
  }
  ...
  GetPolicyDetailByAjustBy(policyID, data) {
    return this.api.execSv<any>(
      'HR',
      'HR',
      'PolicyALBusiness_Old',
