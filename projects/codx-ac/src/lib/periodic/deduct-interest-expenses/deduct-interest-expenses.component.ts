...
import { PopAddDeductInterestExpensesComponent } from './pop-add-deduct-interest-expenses/pop-add-deduct-interest-expenses.component';

@Component({
  selector: 'lib-deduct-interest-expenses',
  templateUrl: './deduct-interest-expenses.component.html',
  styleUrls: ['./deduct-interest-expenses.component.css']
})
export class DeductInterestExpensesComponent extends UIComponent{
  ...
  constructor(
    ...
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10601');
  ...
  ngAfterViewInit() {
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10600');
