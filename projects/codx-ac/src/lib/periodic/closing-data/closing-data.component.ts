...
import { PopAddClosingDataComponent } from './pop-add-closing-data/pop-add-closing-data.component';

@Component({
  selector: 'lib-closing-data',
  templateUrl: './closing-data.component.html',
  styleUrls: ['./closing-data.component.css']
})
export class ClosingDataComponent extends UIComponent{
  ...
  dialog!: DialogRef;
  entityName: any;
  mfFormName: any = 'ClosingData';
  mfGrvName: any = 'grvClosingData';
  funcName: any;
  headerText: any;
  ...
  constructor(
    ...
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP11101');
  ...
  ngAfterViewInit() {
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP11100');
