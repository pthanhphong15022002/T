...
import { PopAddClosingTransactionComponent } from './pop-add-closing-transaction/pop-add-closing-transaction.component';

@Component({
  selector: 'lib-closing-transaction',
  templateUrl: './closing-transaction.component.html',
  styleUrls: ['./closing-transaction.component.css']
})
export class ClosingTransactionComponent extends UIComponent{
  ...
  constructor(
    ...
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10701');
  ...
  ngAfterViewInit() {
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10700');
