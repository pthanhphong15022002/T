...
import { PopAddAllocateToolsComponent } from './pop-add-allocate-tools/pop-add-allocate-tools.component';

@Component({
  selector: 'lib-allocate-tools',
  templateUrl: './allocate-tools.component.html',
  styleUrls: ['./allocate-tools.component.css']
})
export class AllocateToolsComponent extends UIComponent{
  ...
  funcID: any;
  entityName: any;
  mfFormName: any = 'AllocatingTools';
  mfGrvName: any = 'grvAllocatingTools';
  funcName: any;
  headerText: any;
  ...
  constructor(
    ...
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10401');
  ...
  ngAfterViewInit() {
    this.cache.moreFunction(this.mfFormName, this.mfGrvName).subscribe((res: any) =>{
      if (res && res.length) {
        let m = res.find((x) => x.functionID == 'ACP10400');
