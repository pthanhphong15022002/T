  ...
  onTabSelectedDetail(event) {
    switch (event?.selectedIndex) {
      ...
      case 1:
        ...
        let rObjectID = false;
        let lstRequire: any = [];
        if (this.eleGridCashPayment && this.eleGridCashPayment.rowDataSelected == null) {
          rObjectID = true;
        }
            ...
            if (res && res.type != 'error'){
              ...
              if(this.master.data.subType == (this.journal.journalType + '9')){
                if (this.eleGridCashPayment && this.eleGridCashPayment.rowDataSelected) {
                  if (this.eleGridCashPayment.rowDataSelected?.objectID == '' || this.eleGridCashPayment.rowDataSelected.objectID == null) {
