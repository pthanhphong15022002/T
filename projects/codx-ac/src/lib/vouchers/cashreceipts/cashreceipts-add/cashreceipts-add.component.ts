...
export class CashreceiptsAddComponent extends UIComponent implements OnInit {
  ...
  setDefaultLine() {
    ...
    oLine.objectType = this.formCashReceipt.data.objectType;
    ...
    if (indexCashBook > -1) {
      oLine.accountID = this.eleCbxCashBook?.ComponentCurrent?.dataService?.data[indexCashBook].CashAcctID;
    }
    ...
    if (indexReason > -1) {
      oLine.offsetAcctID = this.eleCbxReasonID?.ComponentCurrent?.dataService?.data[indexReason].OffsetAcctID;
