...
import { CodxAcService, fmAdvancePayment } from '../../../codx-ac.service';
import { AC_CashPaymentsLines } from '../../../models/AC_CashPaymentsLines.model';
import { AC_AdvancedPaymentLines } from '../../../models/AC_AdvancedPaymentLines.model';

@Component({
  selector: 'lib-advance-payment-request-add',
  templateUrl: './advance-payment-request-add.component.html',
  styleUrls: ['./advance-payment-request-add.component.css','../../../codx-ac.component.scss'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AdvancePaymentRequestAddComponent extends UIComponent implements OnInit {
  //#region Contructor
  @ViewChild('eleGridAdvancePayment') eleGridAdvancePayment: CodxGridviewV2Component; //? element codx-grv2 lưới Cashpayments
  @ViewChild('formAdvancePayment') public formAdvancePayment: CodxFormComponent; //? element codx-form của AdvancePayment
  ...
  fmAdvancePayment:any = fmAdvancePayment //? form model
  ...
  valueChangeMaster(event: any) {
    ...
    if (event && value && this.formAdvancePayment.hasChange(this.formAdvancePayment.preData,this.formAdvancePayment.data)) { //? nếu data có thay đổi
      this.formAdvancePayment.data.updateColumns = '';
      switch (field.toLowerCase()) {
        //* Lí do, nhân viên
        case 'reasonid':
        case 'objectid':
          this.formAdvancePayment.data.memo = this.getMemoMaster();
          this.formAdvancePayment.setValue('memo',this.formAdvancePayment.data.memo,{onlySelf: true,emitEvent: false});
          break;
      }
    }
  }
  ...
  onAddLine() {
    this.formAdvancePayment.save(null, 0, '', '', false)
      ...
      .subscribe((res: any) => {
        ...
        if (res || res.save || res.update) {
          if (res || !res.save.error || !res.update.error) {
            if (this.eleGridAdvancePayment) { //? nếu lưới advancedpayment có active hoặc đang edit
              this.eleGridAdvancePayment.saveRow((res:any)=>{ //? save lưới trước
                if(res){
                  this.addLine();
                }
  ...
  addLine() {
    ...
    this.eleGridAdvancePayment.addRow(oLine,this.eleGridAdvancePayment.dataSource.length);
  }
  ...
  copyRow(data) {
    data.recID = Util.uid();
    data.index = this.eleGridAdvancePayment.dataSource.length;
    this.eleGridAdvancePayment.addRow(data,this.eleGridAdvancePayment.dataSource.length);
  }
   ...
   */
  deleteRow(data) {
    this.eleGridAdvancePayment.deleteRow(data);
  }
  ...
  calcRemainAmt(totalAmt) {
    ...
    dPayAmt = this.eleGridAdvancePayment.dataSource.reduce((sum, data:any) => sum + data?.dr,0);
   ...
   */
  calcTotalDr(){
    let totalDr = this.eleGridAdvancePayment.dataSource.reduce((sum, data:any) => sum + data?.dr,0);
    this.formAdvancePayment.setValue('totalDR',totalDr,{onlySelf: true,emitEvent: false});
  }
   ...
   * @param event
   */
  onActionGridAdvancePayment(event: any) {
    switch (event.type) {
      ...
      this.dialog.dataService.update(this.formAdvancePayment.data).subscribe();
        break;
      case 'closeEdit': //? khi thoát dòng
      if (this.eleGridAdvancePayment && this.eleGridAdvancePayment.rowDataSelected) {
        this.eleGridAdvancePayment.rowDataSelected = null;
      }
      if(this.eleGridAdvancePayment.isSaveOnClick) this.eleGridAdvancePayment.isSaveOnClick = false; //? trường save row nhưng chưa tới actioncomplete
      setTimeout(() => {
   ...
   */
  refreshGrid() {
    if(this.eleGridAdvancePayment){
      this.eleGridAdvancePayment.dataSource = [];
      this.eleGridAdvancePayment.refresh();
    }
  }
  //#endregion Function
}
