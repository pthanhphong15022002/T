...
export class PdfViewComponent extends UIComponent implements AfterViewInit {
  ...
  @ViewChild('rightToolbar', { static: false }) rightToolbar!: ElementRef;
  ...
  changeAnnotationItem(type: number) {
    if (
      !this.isDisable &&
      this.signerInfo &&
      (this.isActiveToSign || !this.isApprover)
    ) {
      this.holding = type;
      switch (type) {
        case 1:
      ...
      if (['1', '2', '8']) {
        this.innerHtml = `<img src="${this.url}" alt="">`;
      } else {
        this.innerHtml = `<p>${this.url}</p>`;
      }
      let draggable: Draggable = new Draggable(
        this.actions.get(type - 1).nativeElement,
        {
          clone: false,
          dragArea: this.canvas.nativeElement,

          isReplaceDragEle: true,
          dragTarget: this.dragItem.nativeElement,
        }
      );
      let droppable: Droppable = new Droppable(
        this.rightToolbar.nativeElement,
        {
  ...
  checkValidArea(suggestLocation: any) {
