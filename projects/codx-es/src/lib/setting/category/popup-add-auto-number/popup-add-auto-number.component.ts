...
export class PopupAddAutoNumberComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    if (data?.data?.autoAssignRule)
      this.autoDefaultData.autoAssignRule = data?.data?.autoAssignRule;
    ...
    if (this.functionID) {
      ...
      this.api
        .execSv(
          'SYS',
          'ERM.Business.AD',
          'AutoNumberSegmentsBusiness',
          'GetListSegmentsAsync',
          [this.functionID]
        )
        .subscribe((res: any) => {
          if (res) {
    ...
    this.esService
      ...
      .then((fg) => {
        if (fg) {
          ...
          if (!this.autoNoCode) {
            const date = moment(); // had to remove the colon (:) after the T in order to make it work
            let dateInFormat = date.format('YYMMDDHHmmssSSS');
            this.autoNoCode = dateInFormat;
          }
    ...
    if (this.autoNoSegments.length) {
      ...
      if (this.autoNoSetting.lastNumber) {
        ...
        if (this.autoNoSetting.maxLength) {
          if (
            arr.length <
            this.autoNoSetting.maxLength - str.split('').length
          ) {
            let stringFormat =
              strFormat +
              '#'.repeat(
                this.autoNoSetting.maxLength -
                  arr.length -
                  str.split('').length || 0
              ) +
              str;
            ...
            strFormat = this.truncateString(
              strFormat,
              this.autoNoSetting.maxLength - str.split('').length - 1
            );
            let stringFormat =
              strFormat +
              '#'.repeat(
