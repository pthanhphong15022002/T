...
import { ES_File, ES_SignFile } from 'projects/codx-common/src/lib/models/ApproveProcess.model';
import { CodxEsService } from '../codx-es.service';
import { CodxCommonService } from 'projects/codx-common/src/lib/codx-common.service';

@Component({
  selector: 'popup-add-template-sign-file',
  templateUrl: './popup-add-template-sign-file.component.html',
  styleUrls: ['./popup-add-template-sign-file.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PopupAddTemplateSignFileComponent implements OnInit {
  ...
  processIDChange(event) {
    if (event?.field && event?.component && event?.data != '') {
      if (event?.field == 'processID') {
        ...
        if (this.cbbProcess?.length > 0 ) {
          let curProcess = this.cbbProcess.filter(
            (pr: any) => pr.RecID == this.processRecID
          );
          if (curProcess != null && curProcess.length > 0) {
            this.tempSampleProcessName = curProcess[0]?.Title;
            this.processID = curProcess[0]?.ProcessID;
            this.cr.detectChanges();
          }
        }
      }
