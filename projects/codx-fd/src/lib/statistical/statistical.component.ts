...
export class StatisticalComponent extends UIComponent implements OnInit {
    ...
    changeTime(data, f) {
        if (this.countDate > 0) {
            ...
            var TDate = new Date(data.data.getFullYear(), data.data.getMonth() + 1, 0);
    ...
    getDataChartB() {
        ...
        var dt = [];
        let i = 0;
        this.api.execSv<any>("FED", "FED", "CardsBusiness", "GetStatisticBallot1Async", [this.options]).subscribe(res => {
            if (res) {
                this.data = res.card;
                ...
                this.totalBehavior = _.sumBy(res.resultBehaviors, function (o) { return o.count });
                this.getDataSet();
                this.setChartBehavior(res.resultBehaviors);

                listBehavior_Temp.forEach((x) => {
                    dt = [{ idProgressBar: i, behaviorID: x.behaviorID, behaviorName: x.behaviorName, count: x.count, sorting: x.sorting }];
    ...
    caculateTotalRow(columnName) {
        ...
        return this.lstTotalCoin.map(o => o[columnName]).reduce((a, c) => { return a + c })
    }
    ...
    getDataSet() {
        ...
        for (let y = 0; y < this.data.length; y++) {
            arr = [{ key: this.getLabelName(this.data[y]?.key), value: this.data[y]?.value }];
    ...
    getChartA() {
        ...
        this.api.execSv<any>("FED", "FED", "CardsBusiness", "GetStatistical1Async", [this.options])
            .subscribe((res) => {
                var result = [];
                ...
                result.forEach((item) => {
                    ...
                    arrReceived = [{ cardTypeName: item.cardTypeName, receiveQuanlity: item.receiveQuanlity }];
                    arrSended = [{ cardTypeName: item.cardTypeName, sendQuanlity: item.sendQuanlity }];
    ...
    setPredicate() {
        ...
        if (this.fromDate && this.toDate) arrTemp.push({ field: "CreatedOn", value: this.fromDate });
        if (this.cardType && this.cardType != "0") arrTemp.push({ field: "CardType", value: this.cardType });
        if (this.typeBallot) arrTemp.push({ field: "TagType", value: this.typeBallot });
        var i = 0;
        var t = this;
        arrTemp.forEach(function (element, index) {
            ...
            dtValue = dtValue == t.TYPE_Ballot.Ballot_SENDED ? "1" : dtValue == t.TYPE_Ballot.Ballot_RECEIVED ? "2" : dtValue;
            if (element.field == "CreatedOn") {
                spre = "(" + element.field + ">=@" + i + " && " + element.field + "<=@" + (i + 1) + ")";
