...
export class CodxDMService {
  ...
  getThumbnail(data, ext) {
    ...
    } else return `../../../assets/codx/dms/${this.getAvatar(ext)}`; //this.getAvatar(ext);
  }
  ...
  openItem(data: any) {
    if (data.fileName == undefined) {
      ...
      this.fileService
        .GetFiles(data.recID, this.idMenuActive)
        .subscribe(async (res) => {
          this.listFiles = res;
  ...
  filterMoreFunction(e: any, data: any) {
    ...
    var list =
      'DMT0226;DMT0227;DMT0228;DMT0229;DMT0230;DMT0231;DMT0232;DMT0233';
    if (e) {
  ...
  getImage(data) {
    ...
    else return this.getThumbnail(data.thumbnail, data.extension);
  }
