...
export class EditFileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dmSV
      .loadGridView(this.formModel?.formName, this.formModel?.gridViewName)
      .subscribe((item) => {
  ...
  checkCopyRight() {
    if (this.isCopyRight && this.gridViewSetup) {
      var name = [];
      if (this.gridViewSetup['Author'].isRequire && !this.fileEditing.author)
        name.push(this.gridViewSetup['Author'].headerText);
      if (
        this.gridViewSetup['Publisher'].isRequire &&
        !this.fileEditing.publisher
      )
        name.push(this.gridViewSetup['Publisher'].headerText);
      if (
        this.gridViewSetup['PublishYear'].isRequire &&
        !this.fileEditing.publishYear
      )
        name.push(this.gridViewSetup['PublishYear'].headerText);
      if (
        this.gridViewSetup['CopyRights'].isRequire &&
        !this.fileEditing.copyRights
      )
        name.push(this.gridViewSetup['CopyRights'].headerText);
      if (
        this.gridViewSetup['PublishDate'].isRequire &&
        !this.fileEditing.publishDate
      )
        name.push(this.gridViewSetup['PublishDate'].headerText);
  ...
  removeUserRight(index, list: Permission[] = null) {
    if (list == null) {
      if (
        ...
        this.fileEditing.permissions != null &&
    ...
    if ($event.data != null) {
      switch (type) {
        case 'tag':
          ...
          break;
        case 'filename':
          if ($event.data.length > 0) this.fileEditing.fileName = $event.data;
          else this.fileEditing.fileName = '';

          break;
        case 'title':
          if ($event.data.length > 0) this.fileEditing.title = $event.data;
          else this.fileEditing.title = '';
          break;
        case 'subject':
          if ($event.data.length > 0) this.fileEditing.subject = $event.data;
          else this.fileEditing.subject = '';
          break;
        case 'relation':
          if ($event.data.length > 0) this.fileEditing.relation = $event.data;
          else this.fileEditing.relation = '';
          break;
        case 'category':
          this.fileEditing.category = $event.data;
          break;
        case 'language':
          if ($event.data.length > 0) this.fileEditing.language = $event.data;
          else this.fileEditing.language = '';
          break;
        case 'source':
          if ($event.data.length > 0) this.fileEditing.source = $event.data;
          else this.fileEditing.source = '';
          break;
        case 'excerpts':
          if ($event.data.length > 0) this.fileEditing.excerpts = $event.data;
          else this.fileEditing.excerpts = '';
          break;
        case 'description':
          if ($event.data.length > 0)
            this.fileEditing.description = $event.data;
          else this.fileEditing.description = '';
          break;
        case 'authur':
          if ($event.data.length > 0) this.fileEditing.author = $event.data;
          else this.fileEditing.author = '';
          break;
        case 'publishdate':
          if ($event.data != null)
            this.fileEditing.publishDate = $event.data.fromDate;
          else this.fileEditing.publishDate = null;
          break;
        case 'publisher':
          if ($event.data.length > 0) this.fileEditing.publisher = $event.data;
          else this.fileEditing.publisher = '';
          break;
        case 'publishyear':
          if ($event.data != null)
            this.fileEditing.publishYear = $event.data.fromDate;
          else this.fileEditing.publisher = null;
          break;
        //Ngày hết hạn
        case 'expiredOn': {
        ...
        case 'copyrights':
          if ($event.data.length > 0) this.fileEditing.copyRights = $event.data;
          else this.fileEditing.copyRights = '';
          break;
      }
    }
    this.changeDetectorRef.detectChanges();
