...
export class VersionComponent implements OnInit {
  ...
  async handleFileInput(files: FileInfo[]) {
    ...
    this.fileEditing.avatar = `../../..assets/themes/dm/default/img/${this.getAvatar(
      file.name
    )}`;
    this.fileEditing.extension =
  ...
  onDeleteHistory(recID) {
    let index = this.fileEditing.history.findIndex(
  ...
  async versionFile() {
    ...
    if (
      ...
      this.fileEditing.item != undefined &&
      this.fileEditing.item != null
    ) {
        ...
        .subscribe(async (res) => {
          if (res.status == 0) {
            ...
            if (index != -1) {
              ...
              files[
                index
              ].thumbnail = `../../../assets/themes/dm/default/img/${this.dmSV.getAvatar(
                res.data.extension
              )}`; //"../../../assets/img/loader.gif";//res.data.thumbnail;
