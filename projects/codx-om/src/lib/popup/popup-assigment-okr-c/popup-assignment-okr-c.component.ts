...
@Component({
  selector: 'popup-assignment-okr-c',
  templateUrl: './popup-assignment-okr-c.component.html',
  styleUrls: ['./popup-assignment-okr-c.component.scss'],
})
export class PopupAssignmentOKRCComponent extends UIComponent implements AfterViewInit {
  ...
  getKRAndOBParent(){
    this.codxOmService
    .getKRAndOBParent(this.dataOB.recID)
    .subscribe((res: any) => {
      if (res) {
        this.dataOB = res;
  ...
  onSaveForm(){
    ...
    this.api.execSv(
      ...
      OMCONST.BUSINESS.KR,
      'DistributeKRAsync',
      [this.dataOB.recID,this.distributeType,[this.assignmentOKR]]
    ).subscribe(res=>{
      let x= res;
