...
export class ChatBoxComponent implements OnInit {
  ...
  loadGroupInformation(){
    this.api.exec<any>(
      'ERM.Business.WP', 
      'ChatBusiness', 
      'GetGroupInfomationAsync', 
      [this.groupId, this.senderId, this.receiverId]).subscribe((resp: any)=>{
        if(resp){
          this.groupId = resp.groupID;
          this.groupType = resp.groupType;
          this.groupIdChange.emit(this.groupId);
          let sender = resp.members[0].userID == this.user.userID ? resp.members[0] : resp.members[1];
          let receiver = resp.members[0].userID != this.user.userID ? resp.members[0] : resp.members[1];
  ...
  loadChatMessages() {
    this.api.exec<any>(
      ...
      [this.groupId, this.pageSize, this.pageIndex]
      ).subscribe((resp: any[])=>{
        if(resp){
          let messageList = resp[0];

          let lastEle = this.chatMessages.length == 0 ? undefined : this.chatMessages[0];
