...
import { DialogData, NotificationsService } from 'codx-core';
...
export class UpdateStatusPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   this.displayStatus(this.data.status)
  ...
  saveData(){
   ...
   this.tmSv.setStatusTask(this.task.taskID,this.data.status,this.completedOn,this.estimated,this.comment).subscribe(res=>{
    if(res){
