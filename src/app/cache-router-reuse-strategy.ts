...
export class CacheRouteReuseStrategy implements RouteReuseStrategy {
  ...
  retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle | null {
    ...
    if (item.config && route.routeConfig?.children) {
      ...
      if (comRef && comRef.instance) {
        if (comRef.instance.module || comRef.instance.asideTheme) {
          ...
          if(item.config.module){
            this.codxSvc.cacheService.functionList(item.config.module).subscribe((f:any) => {
              if (f) {
                if(f.functionType == "S")
      ...
      if (
        ...
        item.config.codxSvc &&
        (!item.config.codxSvc ||
          item.config.codxSvc.funcID == funcID)
      ) {
  ...
  shouldReuseRoute(
    ...
    console.log("shouldReuseRoute "+this.getPath(curr)+"=>"+this.getPath(future)+":"+ok);
